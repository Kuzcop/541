...
def simulated_annealing(initial_solution, neighbour_gen_fun, objective_fun, iterations, c, seed=-1, show=False):
    ...
    current_eval = objective_fun(current_solution, show)[0]
    count = 0
    # Optimization phase
    i = 0
    while i < iterations:
        next_solution = neighbour_gen_fun(current_solution, rd)
        ex = objective_fun(next_solution)
        next_eval = ex[0]
        acc = ex[1]
        lat = ex[2]
        if next_eval > current_eval or current_eval == 0 or _accept_worse_solution(c, i+1, current_eval, next_eval):
            ...
            with open(file_name, 'a+') as f:
                result = {'HP': current_solution, 'Accuracy': acc, 'Latency': lat, 'Obj': current_eval, 'iter': i}
                print(result, file=f)
...
res, res_eval, iterations_completed = simulated_annealing(cnn_default_hyperparameters, cnn_get_random_neighbouring_solution, cnn_objective, 500, 1, seed=0, show=True)
end_time = time.time()
with open(file_name, 'a+') as f:
    print('-' * 50, file=f)
